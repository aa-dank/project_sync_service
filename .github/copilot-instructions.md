# Copilot Instructions — project_sync_service
...
uv run project-sync migrate      # run SQL migration scripts
...
Sync writes are wrapped in `with db.transaction():`. `Database.__exit__` commits on clean exit, rolls back on exception. For multi-statement operations outside a sync (e.g. migration scripts), use `db.run_migration_file()` which commits after the script.
